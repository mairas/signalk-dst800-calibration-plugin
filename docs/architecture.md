...
`dst-settings` (`src/ui/components/settings-panel.ts`) lists the settings from `GET /api/settings` and fetches that list again after each probe, because it carries what the probe found. It reads every offered, readable setting once per sensor and probe, one request at a time, so the plugin's queue never sees a burst. A capability the probe rejected gets no row, one it got no answer for gets a placeholder offering another probe, and a setting the probe does not ask about, such as the depth offset, is always `yes`. After the first read the server refuses for want of an admin login, it stops reading and says so. A row takes `setting` events from other consoles unless its own request is in flight; that request's answer settles it. How each value reads and each outcome is worded lives in `src/ui/settings.ts`. Simulate mode asks for confirmation before it is turned on.
